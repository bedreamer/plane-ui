...
def show_EC_yc_page(request):
    status, path_pair, reason = get_EC_device_pair_information()
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
        #return render(request, "读取配置参数错误")

    path_pair.append('ec_yaoce.html')
    path = '/'.join(path_pair)

    context = dict()
    status, ec_yc, reason = api.device_get_yc(dev_type)
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
...
def show_EC_yx_page(request):
    status, path_pair, reason = get_EC_device_pair_information()
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
        #return render(request, "读取配置参数错误")

    path_pair.append('ec_yaoxin.html')
    path = '/'.join(path_pair)

    context = dict()
    status, ec_yx, reason = api.device_get_yx(dev_type)
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
        #return render(request, "未通讯/或sid错误")
...
def show_EC_yt_page(request):
    ...
    path_pair.append('ec_yaotiao.html')
    path = '/'.join(path_pair)

    status1, ec_yt, reason1 = api.device_get_yt(dev_type)
    #status, ec_yt, reason = api.device_get_yt("cm")
    if status1 != api.OK:
        reason = {"错误原因": reason1}
        context['reason'] = reason
        return render(request, "07-告警提示页面/alarm_prompt.html", context=context)
...
def show_EC_yk_page(request):
    ...
    path_pair.append('ec_yaokong.html')
    path = '/'.join(path_pair)

    status1, ec_yk, reason1 = api.device_get_yk(dev_type)
    #status, ec_yk, reason = api.device_get_yk("cm")
    if status1 != api.OK:
        reason = {"错误原因": reason1}
        context['reason'] = reason
        return render(request, "07-告警提示页面/alarm_prompt.html", context=context)
...
def ec_yt_cg_page(request):     #把新数据从上面拿下来，再从表里拿到原来的值，比较，不同下发，相同不发
    context = dict()
    #获取页面新值
    ...
    if price == 2:
        if temperature_value != temperature_value1:
            ...
            if status != api.OK:                                                #下发失败，渲染遥调失败页面
                status1, path_pair, reason1 = get_EC_device_pair_information()    #获取遥调失败页面路径
        ...
        if flow_value != flow_value1:
            ...
            #status, pz_list, reason = api.project_list_configure()
            if status != api.OK:
                status1, path_pair, reason1 = get_EC_device_pair_information()
                if status1 != api.OK:
                    return render(request, "07-告警提示页面/alarm_prompt.html")
                    # return render(request, "读取配置参数错误")
                else:
...
urlpatterns = [
    path('ec/yc/', show_EC_yc_page, name="ec yc page"),
    path('ec/yx/', show_EC_yx_page, name="ec yx page"),
    path('ec/yt/', show_EC_yt_page, name="ec yt page"),
    path('ec/yk/', show_EC_yk_page, name="ec yk page"),
    path('ec/yt/success/', ec_yt_cg_page, name="ec设备遥调成功页面"),
    path('ec/yt/fail/', ec_yt_sb_page, name="ec设备遥调失败页面"),
    path('ec/yk/success/', ec_yk_cg_page, name="ec设备遥控成功页面"),
    path('ec/yk/fail/', ec_yk_sb_page, name="ec设备遥控失败页面"),
]
urls= (urlpatterns, 'ec', 'ec' )
