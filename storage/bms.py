...
def get_BMS_device_pair_information():
    info = ["04-BMS页面"]
...
def show_BMS_yc_page(request):
    status, path_pair, reason = get_BMS_device_pair_information()
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
        #return render(request, "读取配置参数错误")

    path_pair.append('bms_yaoce.html')
    path = '/'.join(path_pair)

    context = dict()
    status, bms_yc, reason = api.device_get_yc(dev_type)
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
...
def show_BMS_yx_page(request):
    status, path_pair, reason = get_BMS_device_pair_information()
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
        #return render(request, "读取配置参数错误")

    path_pair.append('bms_yaoxin.html')
    path = '/'.join(path_pair)

    context = dict()
    status, bms_yx, reason = api.device_get_yx(dev_type)
    if status != api.OK:
        return render(request, "07-告警提示页面/alarm_prompt.html")
        #return render(request, "未通讯/或sid错误")
...
def show_BMS_yt_page(request):
    ...
    path_pair.append('bms_yaotiao.html')
    path = '/'.join(path_pair)

    status1, bms_yt, reason1 = api.device_get_yt(dev_type)
    if status1 != api.OK:
        reason = {"错误原因": reason1}
        context['reason'] = reason
        return render(request, "07-告警提示页面/alarm_prompt.html", context=context)
    ...
    context['bms_yt'] = bms_yt
    return render(request, path, context=context)


def show_BMS_yk_page(request):
    context = dict()
    status, path_pair, reason = get_BMS_device_pair_information()
    if status != api.OK:
        reason = {"错误原因": reason}
    ...
    path_pair.append('bms_yaokong.html')
    path = '/'.join(path_pair)

    status1, bms_yk, reason1 = api.device_get_yk(dev_type)
    if status1 != api.OK:
        reason = {"错误原因": reason1}
        context['reason'] = reason
        return render(request, "07-告警提示页面/alarm_prompt.html", context=context)
    ...
    context['bms_yk'] = bms_yk
    return render(request, path, context=context)


def bms_yt_cg_page(request):        #把新数据从上面拿下来，再从表里拿到原来的值，比较，不同下发，相同不发
    context = dict()
    # 获取页面新值
    ...
    if price == 2:
        if temperature_value != temperature_value1:
            ...
            if status != api.OK:  # 下发失败，渲染遥调失败页面
                status1, path_pair, reason1 = get_BMS_device_pair_information()  # 获取遥调失败页面路径
        ...
        if flow_value != flow_value1:
            ...
            # status, pz_list, reason = api.project_list_configure()
            if status != api.OK:
                status1, path_pair, reason1 = get_BMS_device_pair_information()
                if status1 != api.OK:
                    return render(request, "07-告警提示页面/alarm_prompt.html")
                    # return render(request, "读取配置参数错误")
                else:
...
def bms_yt_sb_page(request):
    ...
    if status != api.OK:
        ...
        path_pair.append('bms_yt_sb.html')
        path = '/'.join(path_pair)
        return render(request, path, context=context)


def bms_yk_cg_page(request):
...
def bms_yk_sb_page(request):
    ...
    if status != api.OK:
        ...
        path_pair.append('bms_yk_sb.html')
        path = '/'.join(path_pair)
        return render(request, path, context=context)

urlpatterns = [
    path('bms/yc/', show_BMS_yc_page, name="bms yc page"),
    path('bms/yx/', show_BMS_yx_page, name="bms yx page"),
    path('bms/yt/', show_BMS_yt_page, name="bms yt page"),
    path('bms/yk/', show_BMS_yk_page, name="bms yk page"),
    path('bms/yt/success/', bms_yt_cg_page, name="bms设备遥调成功页面"),
    path('bms/yt/fail/', bms_yt_sb_page, name="bms设备遥调失败页面"),
    path('bms/yk/success/', bms_yk_cg_page, name="bms设备遥控成功页面"),
    path('bms/yk/fail/', bms_yk_sb_page, name="bms设备遥控失败页面"),
]
